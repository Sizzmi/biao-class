...
class Route {
  ...
  load_route_config() {
    ...
    for (var name in route_list) {
      ...
      path_arr.forEach((segment, index) => {
        let is_param = segment.startWith(':');
   ...
   * @param {string} opt 配置项，如：{force:false}
   */
  go(hash, opt = null) {
    ...
    if (!this.current.el)
      throw new ReferenceError(`请配置route${this.current.name}的el参数`);

    /**渲染新页面 */
    this.render();

    /**如果外部使用者想路由切换完毕时做额外的事情就调用它给我们传的函数 */
