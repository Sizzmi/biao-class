var Tool = function () {//构造函数模式：用的时候需要new一下：
  ...
  //每次只需要判断this.flag是否存在就可以了：如果存在就是标准浏览器，如果不存在就是
};
Tool.prototype = {//方法是定义在Tool的prototype上的；
  constructor: Tool,//重写prototype后，prototype的constructor 已经不是原来的Tool了：
  // 需要手动给他强制写会到Tool上去;
  /**经常用到的函数 */
  ...
  toJSON: function (jsonStr) {//将json字符串转化为json对象
    var jsonObj = null;
    try {
      jsonObj = JSON.parse(jsonStr);//如果支持JSON。parse则直接使用JSON。parse将JSON字符串转换为JSON对象。
    } catch (e) {
  ...
  getEleChildren: function (parent, tagName) {//获取指定节点名的元素节点们：第二个参数如果不传，则返回obj下的所有子元素节点：
    ...
    for (var i = 0; i < allNode.length; i++) {
      var cur = allNode[i];
      if (cur.nodeType === 1) {
        if (tagName) {//tahName不可能为0，flase之类的数：所以可以用if直接判断传进来的参数：
  ...
  getSiblings: function (curEle) {//获取所有的兄弟们（除了弟弟）
  ...
  insertAfter: function (nexEle, oldEle) {//在目标元素oldEle后面插入新元素newEle:如果没有传指定元素，则直接添加在后面
  ...
  text: function (ele, str) {//处理innerText和textContent的兼容性：传一个参数是获取：2个参数是设置：
    if (ele && eke, nodeType && ele.nodeType == 1) {
  ...
  setCss: function (curEle, attr, value) {
    ...
    if (typeof value === "undefined") {
      ...
      var reg = /^(?:margin|pddding|border|float|position|display|background|backgroundColor)$/;
  ...
  setGroupCss: function (curEle, cssObj) {//给元素设置一组属性cssObj是一个对象类型：
  ...
  removeClassName: function (curEle, cName) {//移出类样式的方法：
...
~function () {//在DOM库上增加方法，同时不影响原来的方法：在类的原型上增加方法：
  ...
  strPro.myQueryURLParameter = function () {//是否有效URL
    var reg = /([^?&=]+)=([^?&=]+)/g, obj = {};
    this.replace(reg, function () {
