...
function on(ele, type, fn) {
  /**约定type如果以self开头的字符串，就是自定义事件，如果不是的，就是系统事件 */
  if (/^self/.test(type)) {//判断是否是自定义事件，这里是自定义事件：
    ...
    if (ele.addEventListener) {
      ...
      if (!ele['aEvent' + type]) {
        ...
        ele.attachEvent("on" + type, function () { run.call(ele) })//放在这里，因为是在if里，所以室友条件执行的，而且这里是只执行一次：防止重复绑定；
      }
...
function run() {
  ...
  for (var i = 0; i < a.length;) {
    /**下面是防止数组塌陷的 */
    if (typeof a[i] == "function") {//这个是和off相呼应的，off里有为null的值；
